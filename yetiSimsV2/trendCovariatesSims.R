...
simControl<-readRDS("simsToDo.rds") %>% .[simNum==whichSim]
...
############################ NPH added section below on 20 Mar 2017
if(simControl$stage[s]=="yoy" & simControl$covariates[s]==F) load("bkt_out_yoy_M1.RData") # YOY model without covars (contains "out2")
if(simControl$stage[s]=="yoy" & simControl$covariates[s]==T) load("bkt_out_yoy_M2.RData") # YOY model with covars (contains "out2")
if(simControl$stage[s]=="adult" & simControl$covariates[s]==F) load("bkt_out_adult_M1.RData") # Adult model without covars (contains "out2")
if(simControl$stage[s]=="adult" & simControl$covariates[s]==T) load("bkt_out_adult_M2.RData") # Adult model with covars (contains "out2")
############################ NPH added section above on 20 Mar 2017
