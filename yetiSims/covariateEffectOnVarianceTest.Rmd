...
```
```{r,echo=F}
plot(density(sdNoCov),main="",xlab="Variance of Generated Response Variable")
points(density(sdCov),type='l',col="blue")
legend(1,2,c("no covariates","covariates"),lty=1,col=c("black","blue"),bty='n')
```
...
plot(density(sdNoCov),main="",xlab="Variance of Generated Response Variable")
points(density(sdCov),type='l',col="blue")
legend(0.85,2.9,c("no covariates","covariates"),lty=1,col=c("black","blue"),bty='n')
```
...
In our case, we believe that the true data generating model includes covariates, so generating the data using covariates and then fitting it with or without covariates may be the best route. This gives a more fair comparison because the simulated data would have the same amount of noise ragardless of which model we are fitting to them. The only difference is the way that we fit the data.

The model that includes covariates should have lower residual variation when the covariates are included in the data generation part, as is the case for this linear regression and the YOY models. This should improve trend detection when covariates are included.
```{r}
#without covariates
summary(a1)$sigma
...
###So I propose using covariates to generate the data in all cases and then fitting the models with or without covariates. 