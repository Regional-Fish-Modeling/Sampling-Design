...
#   .[simNum==whichSim]
simControl<-readRDS("yetiSims/simsToDo.rds")
simControl<-simControl[simNum==whichSim]
...
for(s in 1:nrow(simControl)){
  ...
  model = ifelse(includeCovs,"yetiSims/trendModelCov.r","yetiSims/trendModelNoCov.r")
...
#               ".RData"))
#   
  load(paste0("yetiSims/",simControl$stage[s],
              ifelse(includeCovs,"Cov","NoCov"),
              ".RData"))
...
# parameters to save
      ...
      #generate environment (model fit with z-score standardized data)
      if(includeCovs){
